...
const Main: FC<Props> = ({
  ...
  const sortedPlaceCardsList = useMemo(() => sortOffers(sortType, placeCardsList), [
    sortType,
    placeCardsList,
  ]);
  ...
  const {
    activeItem,
    handleSetActiveItem: setActiveItem,
    handleRemoveActiveItem: removeActiveItem,
  } = useActiveItem();

  const place = useMemo(() => locations.find((cityInfo) => cityInfo.name === city), [
    locations,
    city,
  ]);
  ...
  return (
        <>
            <main className="page__main page__main--index">
              ...
              <CitiesTabsList locations={locations} setCity={setCity} activeCity={city} />
...
const WrappedMain = withSortMenu(memo(Main));
